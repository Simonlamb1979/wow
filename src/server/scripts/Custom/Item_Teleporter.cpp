...
class ItemTeleporter : public ItemScript
{
public:
    ItemTeleporter() : ItemScript("ItemTeleporter") { }

    Item_Teleporter tmp;

    bool OnItemHello(Player* player, Item* pItem) override
    {
        ...
        SendGossipMenuFor(player, TEXT_HELLO, pItem->GetGUID());
        return true;
    }

    void SendDefaultMenu(Player *player, Item *pItem, uint32 sender)
    {
        uint8 Expansion = player->GetSession()->GetExpansion();
        if (sender <= 1000) //显示次级菜单
        {
            ...
            if (result)
            {
                ...
                SendGossipMenuFor(player, TEXT_FIRST, pItem->GetGUID());
            }
        }
        // 执行具体菜单动作
        if (sender >= 1000)
        {
            ...
            if (result)
            {
                ...
                case 0: //菜单跳转,菜单页参数存于Param字段中
                    if (tmp.Param)
                    {
                        ...
                        if (result) {
                            ...
                            SendGossipMenuFor(player, TEXT_SECOND, pItem->GetGUID());
                        }
                    }
                    else
                    {
                        OnItemHello(player, pItem);
                    }
                    break;
                case 1: //一般传送功能
                    CloseGossipMenuFor(player);
                ...
                case 6: //播放电影
                    ...
                    break;
                case 7: //随身银行
                    player->GetSession()->SendShowBank(pItem->GetGUID());
                    CloseGossipMenuFor(player);
                    break;
                case 8: //技能提升
                ...
                case 12://版本说明
                    OnItemHello(player, pItem);
                    player->Say(" |cff888888现在版本为T-725_V6.1|R ", LANG_UNIVERSAL);
                    break;
                case 13:// 变身
                    CloseGossipMenuFor(player);
                ...
                case 28:
                    ...
                    break;
                case 29:
                    if (!player->hasQuest(tmp.x))
                    {
                        player->Say(" |cff888888你还没有开始这个任务。|R ", LANG_UNIVERSAL);
                    }
                    else
                    {
                        player->CompleteQuest(tmp.x);
                        player->Say(" |cFF2E8B57你已完成这个任务。|R ", LANG_UNIVERSAL);
                    }
                    break;
    ...
    bool OnItemSelect(Player *player, Item *pItem, uint32 sender, uint32 action) override
    {
        // Main menu
        player->PlayerTalkClass->ClearMenus();
        if (sender == GOSSIP_SENDER_MAIN)
            SendDefaultMenu(player, pItem, action);
        return true;
    }
};
